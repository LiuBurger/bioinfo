...
batchsize = 1024
trainloader = DataLoader(trainset, batch_size=batchsize, shuffle=True, drop_last=True, num_workers=6, collate_fn=collate_fn)
validloader = DataLoader(validset, batch_size=batchsize, shuffle=False, drop_last=False, num_workers=6, collate_fn=collate_fn)

prot_model = ResNet([1, 1, 1, 1]).cuda()
    ...
    for epoch in range(num_epochs):
        ...
        with pt.no_grad():  # 不计算梯度，节省内存
            for seq, ptr, lab in validloader:
                ...
                acc = float(
                    pt.sum(label == pt.argmax(lab, axis=1,
                                              keepdim=True))) / batchsize
                running_acc.append(acc)

        logging.info(
            "epoch: %d\n loss: %.4f \n acc: %.2f%%" %
            (epoch + 1, pt.mean(pt.tensor(running_loss)).detach().numpy(),
