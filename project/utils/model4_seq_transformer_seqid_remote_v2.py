...
class SharedSeqGraphEncoder(nn.Module):
    ...
    def forward(self, seq: pt.Tensor, mask: pt.Tensor, graph):
        x = graph.x.float()                                              # [num_nodes, node_feat_dim]
        ...
        x = pt.cat([node_seq_feat, x], dim=-1)                           # [num_nodes, seq_dim + node_feat_dim]
        x = self.gnn(x, edge_index, edge_attr=edge_attr)
        x = self.node_out_norm(x)

        graph_global = self.attn_pool(x, batch)                          # [B, graph_hidden_dim]
...
class DualEncoderRetriever(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.hidden_dim = hidden_dim                    # 这里表示 Transformer seq_dim
        ...
        # InfoNCE 检索头：输入图级表示，因此维度是 graph_hidden_dim
        ...
        # SEQID 回归头：输入序列级 Transformer 表示，因此维度是 seq_dim
        ...
        # remote-homology score 回归头：输入图级表示，因此维度是 graph_hidden_dim
