...
class SharedSeqGraphEncoder(nn.Module):
    ...
    同时返回：
    - seq_global: 供 SEQID 回归
    - graph_global: 供 remote-score 回归与检索
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        self.node_out_norm = nn.LayerNorm(hidden_dim)

        gate_hidden = attn_gate_hidden if attn_gate_hidden is not None else max(32, hidden_dim // 2)
        ...
        self.graph_out_norm = nn.LayerNorm(hidden_dim)
    ...
    def forward(self, seq: pt.Tensor, mask: pt.Tensor, graph):
        ...
        seq_out = self.seq_encoder(seq, mask)                          # [B, L, seq_dim]
        seq_global = SequenceTransformer.masked_mean(seq_out, mask)    # [B, seq_dim]
        ...
        node_seq_feat = seq_flat[flat_idx]                             # [num_nodes, seq_dim]

        x = pt.cat([node_seq_feat, x], dim=-1)                         # [num_nodes, hidden_dim]
...
class DualEncoderRetriever(nn.Module):
    def __init__(
        ...
        # 一个输出头做 SEQID 回归（基于 Transformer 的序列表征）
        self.seqid_head = PairRegressionHead(
            input_dim=hidden_dim + node_feat_dim,
            hidden_dim=max(32, hidden_dim // 2),
            dropout=dropout,
        )

        # 一个输出头做 remote-homology score 回归（基于图级表示）
        self.remote_head = PairRegressionHead(
            input_dim=hidden_dim,
            hidden_dim=max(32, hidden_dim // 2),
            dropout=dropout,
        )
    ...
    def _encode_shared(self, seqs: pt.Tensor, masks: pt.Tensor, graphs):
        return self.shared_encoder(seqs, masks, graphs)

    def encode_query_backbone(self, query_seqs, query_masks, query_graphs):
        return self._encode_shared(query_seqs, query_masks, query_graphs)

    def encode_cand_backbone(self, cand_seqs, cand_masks, cand_graphs):
        return self._encode_shared(cand_seqs, cand_masks, cand_graphs)

    def encode_query(self, query_seqs, query_masks, query_graphs):
        _, q_graph = self.encode_query_backbone(query_seqs, query_masks, query_graphs)
        return self.rank_head(q_graph)

    def encode_cand(self, cand_seqs, cand_masks, cand_graphs):
        _, c_graph = self.encode_cand_backbone(cand_seqs, cand_masks, cand_graphs)
        return self.rank_head(c_graph)
    ...
    def forward(self, batch: Dict[str, pt.Tensor]):
        q_seq, q_graph = self.encode_query_backbone(
            batch['query_seqs'], batch['query_masks'], batch['query_graphs']
        )
        p_seq, p_graph = self.encode_cand_backbone(
        ...
        remote_score_pred = self.remote_head(q_graph, p_graph)

        return {
            'info_nce_loss': info_nce_loss,
            'seqid_pred': seqid_pred,
            'remote_score_pred': remote_score_pred,
            'pos_score': pos_score,
            'neg_score': neg_score,
            'logits': logits,
            'q_seq': q_seq,
...
class MultiTaskOptimizer(pt.optim.AdamW):
    def __init__(
        ...
        alpha_remote: float = 0.2,
        alpha_seqid: float = 0.1,
        aux_loss_type: str = 'smooth_l1',
        aux_beta: float = 0.05,
    ):
        self.alpha_remote = float(alpha_remote)
    ...
    def combine_loss(
        self,
        rank_loss: pt.Tensor,
        remote_score_pred: pt.Tensor,
        remote_score_true: pt.Tensor,
        seqid_pred: pt.Tensor,
        seqid_true: pt.Tensor,
    ):
        remote_aux = self._reg_loss(remote_score_pred, remote_score_true)
        seqid_aux = self._reg_loss(seqid_pred, seqid_true)
        total_loss = rank_loss + self.alpha_remote * remote_aux + self.alpha_seqid * seqid_aux
        return total_loss, remote_aux, seqid_aux
    ...
    def _step(
        ...
        loss, remote_aux, seqid_aux = self.combine_loss(
            rank_loss,
            remote_score_pred,
            remote_score_true,
            seqid_pred,
