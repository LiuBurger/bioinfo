...
from utils.data_rope import ProteinDataset, QueryHomologyDataset, collate_fun_train, collate_fun_emb
from utils.model_rope import DualEncoderRetriever, MultiTaskOptimizer
from utils.tools import build_idx, calculate_remote_homology_score, gen_embeddings, move_batch_to_device


def parse_args():
    parser = argparse.ArgumentParser(description='Shared-trunk SeqTransformer + GINE dual-encoder training config (TM-score regression + SEQID regression)')
    ...
    parser.add_argument('--alpha_tmscore', type=float, default=0.2)
...
class Trainer:
    def __init__(self, model, config: argparse.Namespace):
        ...
        self.optimizer = MultiTaskOptimizer(
            ...
            alpha_tmscore=config.alpha_tmscore,
    ...
    def _evaluate(self, test_loader, lib_loader, test_map, test_set, datalib):
        ...
        embs_lib = gen_embeddings(
            model=self.model,
            loader=lib_loader,
            device=self.device,
            normalize_output=False,
        )
        embs_test = gen_embeddings(
            model=self.model,
            loader=test_loader,
            device=self.device,
            normalize_output=False,
        ...
        try:
            for epoch in range(self.config.epochs):
                ...
                for batch in tqdm(train_loader, unit='batch'):
                    batch_gpu = move_batch_to_device(batch, self.device)
                    ...
                    tmscore_mae = (output['tmscore_pred'].detach() - batch_gpu['tmscores']).abs().mean()
                    ...
                    running['tmscore_aux'] += float(step_stats['tmscore_aux'])
                    running['seqid_aux'] += float(step_stats['seqid_aux'])
                    running['tmscore_mae'] += float(tmscore_mae.detach())
                ...
                self._log(
                    ...
                    f"TM-score aux: {running['tmscore_aux'] / max(steps, 1):.6f} | "
                    f"SEQID aux: {running['seqid_aux'] / max(steps, 1):.6f} | "
                    f"TM-score MAE: {running['tmscore_mae'] / max(steps, 1):.6f} | "
                    f"SEQID MAE: {running['seqid_mae'] / max(steps, 1):.6f} | "
                    f"Grad norm: {running['grad_norm'] / max(steps, 1):.6f} | "
                    f"AlphaTM: {self.optimizer.alpha_tmscore:.4f} | "
